...
def getVideoScenes(videoPath):
    ...
    if len(scene_list) == 0:
        ...
        scene_list = [(timestamp[0].get_seconds(), timestamp[1].get_seconds())
                      for timestamp in scene_list]
...
def get_suitable_scenes(video_list, scene_list, face_detector, min_frames):
    ...
    for i, v_path in enumerate(tqdm(video_list, desc="Filtering scenes...")):
        suitable = True
        video = video_list[i]
        scene = scene_list[i]
        ...
        # Removes non suitable video and continues
        if not suitable:
            cap.release()
            os.remove(v_path)
            continue
    ...
    return suitable_videos, suitable_scenes


def convert_video_to_audio_ffmpeg(video_file, output_ext="wav"):
    """Extracts audio from video to .wav format, returns path of the resulting audio"""
    filename = os.path.basename(os.path.realpath(video_file)).split('.')[0]
    wav_path = os.path.join(os.getcwd(), "temp", f"{filename}.{output_ext}")
    # Gets working directory and extracts audio
    subprocess.call(["ffmpeg", "-y", "-i", video_file, "-vn", "-ac", "1", "-ar", "16000", "-acodec",
                     "pcm_s16le", "-loglevel", "quiet",  wav_path],
...
def extractBiggestFace(img, detector):
    ...
    try:
        xmin, ymin, xmax, ymax = int(cntr[0]), int(
            cntr[1]), int(cntr[2]), int(cntr[3])
        # Cara detectada, reescalamos
        resImage = cv2.resize(img[max(ymin, 0):ymax, xmin:xmax], (112, 112))
...
def saveFaceCrops(videoPath, detector):
    ...
    facePos = []
    while success:
        resImage, (xmin, ymin, xmax, ymax) = extractBiggestFace(
            image, detector)
    ...
    while success:
        ...
        if not faces_detected and faces is not None:  # No hay caras aún
            ...
            for d in range(len(faces)):
                ...
                frames[d] = [count]
        # Vamos a comparar a qué cara pertenece según posición (implementar threshold por si nueva cara)
        else:
            if faces and faceCoords:
                for f in range(len(faces)):
                    minDist = 2**32
                    predFace = -1
                    for key in facePos.keys():
                        res = np.linalg.norm(np.asarray(
                            faceCoords[f])-np.asarray(facePos[key][-1]))
                        if res < minDist:
                            predFace = key
                            minDist = res
...
def checkVideoDuration(videoPath):
    res = subprocess.check_output(["ffprobe", "-v", "error", "-show_entries",
...
def padVideo(video, center, nframes):
    ...
    if center < nSideFrames:  # Necesitamos hacer padding por la izquierda
        ...
        video = F.pad(video, (0, 0, 0, 0, padAmount, 0),
                      "constant", 0)  # Padding al principio
        ini = 0
        fin = nframes
    if center+nSideFrames >= videoFrames:  # Necesitamos hacer padding al final
        padAmount = (nSideFrames+center) - videoFrames+1
        video = F.pad(video, (0, 0, 0, 0, 0, padAmount),
                      "constant", 0)  # Padding al final
...
def padAudio(audio, label, center, nframes, videoFrames):
    ...
    if center < nSideFramesAudio:  # Necesitamos hacer padding por la izquierda
        ...
        audio = F.pad(audio, (0, 0, padAmount, 0),
                      "constant", 0)  # Padding al principio
        ini = 0
        fin = nframes*4
    if center+nSideFramesAudio+4 >= audioFrames:  # Necesitamos hacer padding al final
        ...
        audio = F.pad(audio, (0, 0, 0, padAmount),
                      "constant", 0)  # Padding al final
...
def saveFullVideo(videoName, inputVideo, audioPath, totalScores, faceFrames, predArray, facePos):
    ...
    for speakerN in totalScores.keys():
        ...
        for i, fr in enumerate(faceFrames[speakerN]):
            try:
                ...
                cv2.putText(image, "{:.2f}".format(
                    totalScores[speakerN][i]*100), (xmin, ymin-5), cv2.FONT_HERSHEY_SIMPLEX, 0.35, color, 1)
            except:
                pass
    cap.release()
    cv2.destroyAllWindows()
